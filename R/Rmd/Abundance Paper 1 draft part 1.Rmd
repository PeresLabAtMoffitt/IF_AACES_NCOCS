...
`r emo::ji("question_mark")` Do you want high with White or with Black? or each comparison? I thnk it is the later but what is the thought behind that?
...
tbl1 <- coxph(Surv(time = markers_ROIclust_t$timelastfu_new, 
  ...
  tbl_regression(exponentiate = TRUE,
                 include = !CD3:race) %>% bold_p(t = .05) %>% add_nevent(location = "level")
tbl2 <- coxph(Surv(time = markers_ROIclust_t$timelastfu_new, 
...
The regression relies on the assumption of no unobserved confounders.  
Here I am just trying things. I needed to play around because got errors and discovered that:  
covar must be numeric  
Can only do lm?  
```{r, include=TRUE}
# model <- coxph(Surv(time = markers_ROIclust_o$timelastfu_new,
#                    event = markers_ROIclust_o$surv_vital) ~ CD3 + stage + refage + race + CD3*race, cluster = suid, 
#                data =  markers_ROIclust_o)

model_s <- lm(tumor_total_cells ~ surv_vital+CD3 + stage + refage + race, 
            data =  markers_ROIclust_o)
model_s
model_sens <- sensemakr(model = model_s, 
                        treatment = "surv_vital",
                        benchmark_covariates = "refage", # names of covariates that will be used to bound the plausible strength of the unobserved confounders - the strongest covariate that we know of
                        kd = 1:3) # how many times the confoundres can be compare to benchmark_covariates

print(model_sens)
summary(model_sens)
plot(model_sens)
# Need covar must be numeric
markers_ROIclust_o$race <- ifelse(markers_ROIclust_o$race == "White", 0, 1)
markers_ROIclust_o$CD3 <- ifelse(markers_ROIclust_o$CD3 == "low", 0, 1)

model_s <- lm(surv_vital ~ CD3 + stage + refage + race, 
            data =  markers_ROIclust_o)
model_s
model_sens <- sensemakr(model = model_s, 
                        treatment = "CD3",
                        benchmark_covariates = "race",
                        kd = 1:3)
print(model_sens)
summary(model_sens)
plot(model_sens)

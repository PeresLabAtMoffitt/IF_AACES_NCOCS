...
ROI_global %>% 
  ...
  distinct(suid, .keep_all = TRUE) %>% 
  mutate(prop = eur_prop) %>% 
  mutate(prop1 = eur_prop+asian_prop) %>% 
  
  arrange((prop)) %>%
  # select(suid, eur_prop, afr_prop, asian_prop, prop)
  pivot_longer(cols = c(ends_with("_prop")), values_to = "ancestry_prop", names_to = "ancestry_group") %>% 
  mutate(ancestry_group = factor(ancestry_group, levels = c("asian_prop", "afr_prop", "eur_prop"))) %>% 
  # filter(!is.na(ancestry_prop)) %>%
  ggplot(aes(x= fct_reorder(suid, (prop1)), y = ancestry_prop, 
             fill = ancestry_group, color = ancestry_group)) +
...
ROI_global %>% 
  distinct(suid, .keep_all = TRUE) %>% 
  pivot_longer(cols = c("brca1", "brca2"), values_to = "brca_prop", names_to = "brca_group") %>% 
  filter(!is.na(brca_prop)) %>% 
  select(brca_prop, brca_group) %>% 
  ggplot(aes(x= brca_group, fill = brca_prop)) +
...
markers_ROIclust_o <- ROI_global %>%
  filter(!is.na(brca1)) %>%
  filter(intratumoral_i_vs_peripheral_p_ == "Intratumoral") %>% 
  rename(CD11b_CD15 = CD11b_CD15_total)
...
markers_ROIclust_t <- ROI_global %>% 
    filter(intratumoral_i_vs_peripheral_p_ == "Intratumoral") %>% 
  rename(CD11b_CD15 = CD11b_CD15_tumor)

coxph(Surv(time = markers_ROIclust_t$timeint_fu, 
                   event = markers_ROIclust_t$surv_vital) ~ CD11b_CD15 + brca1 + stage + refage + race, cluster = suid, data =  markers_ROIclust_t) %>%
  tbl_regression(exponentiate = TRUE,
                 include = CD11b_CD15) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
```
## Stroma peripheral
...
markers_ROIclust_s <- ROI_global %>% 
  filter(!str_detect(suid, paste0(remove_id, collapse = "|")), 
         intratumoral_i_vs_peripheral_p_ == "Peripheral") %>% 
  rename(CD3_CD8 = CD3_CD8_stroma)

coxph(Surv(time = markers_ROIclust_s$timeint_fu, 
                   event = markers_ROIclust_s$surv_vital) ~ CD3_CD8 + brca1 + stage + refage + race, cluster = suid, data =  markers_ROIclust_s) %>%
  tbl_regression(exponentiate = TRUE,
                 include = CD3_CD8) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
```
<br>
...
# HR adjusted by brca1
## White pop restricted to those who have brca1 info
## Overall intratumoral
```{r}
markers_ROIclust_o <- ROI_global %>%
  filter(!is.na(brca1)) %>%
...
coxph(Surv(time = markers_ROIclust_t$timeint_fu, 
                   event = markers_ROIclust_t$surv_vital) ~ CD11b_CD15 + brca1 + stage + refage, cluster = suid, data =  markers_ROIclust_t) %>%
  tbl_regression(exponentiate = TRUE,
                 include = CD11b_CD15) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
```
## Stroma peripheral
...
coxph(Surv(time = markers_ROIclust_s$timeint_fu, 
                   event = markers_ROIclust_s$surv_vital) ~ CD3_CD8 + brca1 + stage + refage, cluster = suid, data =  markers_ROIclust_s) %>%
  tbl_regression(exponentiate = TRUE,
                 include = CD3_CD8) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
```
<br>
...
# HR adjusted by brca1
## Black pop restricted to those who have brca1 info
## Overall intratumoral
```{r}
markers_ROIclust_o <- ROI_global %>%
  filter(!is.na(brca1)) %>%
...
# pdf(paste0(path, "/Survivals/Immune cell clusters/OS CD3 grp matched pop.pdf"), width = 8, height = 7)
myplot <- ggsurvplot(myplot, data = clin_surv,
           title = "Survival analysis on White matched population", subtitle = "Grouping based on peripheral overall CD3+ cells",
           font.main = c(14, "bold", "black"), font.subtitle = c(6, "plain", "black"),
           xlab = "Time (days)", legend.title = "", 
           legend.labs = c("High", "Low"),
           palette = c("grey27", "grey77"),
...
# pdf(paste0(path, "/Survivals/Immune cell clusters/OS CD3 grp matched pop.pdf"), width = 8, height = 7)
myplot <- ggsurvplot(myplot, data = clin_surv,
           title = "Survival analysis on Black matched population", subtitle = "Grouping based on peripheral overall CD3+ cells",
           font.main = c(14, "bold", "black"), font.subtitle = c(6, "plain", "black"),
           xlab = "Time (days)", legend.title = "", 
           legend.labs = c("High", "Low"),
           palette = c("grey27", "grey77"),
...
## Overall "Surv ~ CD3_total + stage + refage + race, cluster = suid"
...
fit2 <- coxph(Surv(time = timeint_fu, event = surv_vital) ~ CD3_total + stage + refage + race, cluster = suid, data = clin_surv)
...
## White "Surv ~ CD3_total + stage + refage, cluster = suid"
### Intratumoral total CD3
```{r}
clin_surv <- ROI_global %>% 
  filter(intratumoral_i_vs_peripheral_p_ == "Intratumoral") %>% 
...
fit2 <- coxph(Surv(time = timeint_fu, event = surv_vital) ~ CD3_total + stage + refage, cluster = suid, data = clin_surv)
ggadjustedcurves(fit2, data = clin_surv, 
                 method = "average", 
                 variable = "CD3_total",
                 palette = c("grey27", "grey77"), 
...
## Black "Surv ~ CD3_total + stage + refage, cluster = suid"
### Intratumoral total CD3
```{r}
clin_surv <- ROI_global %>% 
  filter(intratumoral_i_vs_peripheral_p_ == "Intratumoral") %>% 

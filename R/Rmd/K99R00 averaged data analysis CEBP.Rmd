...
# HR adjusted by brca1
## Overall pop restricted to those who have brca1 info
## Overall intratumoral
```{r}
markers_ROIclust_o <- markers_ROI %>%
  filter(!is.na(brca1)) %>%
  rename(CD11b_CD15 = CD11b_CD15_total.i)

coxph(Surv(time = markers_ROIclust_o$timeint_fu, 
                   event = markers_ROIclust_o$surv_vital) ~ CD11b_CD15 + brca1 + stage + refage + race, cluster = suid, data =  markers_ROIclust_o) %>%
  tbl_regression(exponentiate = TRUE,
                 include = CD11b_CD15) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
```
...
coxph(Surv(time = markers_ROIclust_o$timeint_fu, 
                   event = markers_ROIclust_o$surv_vital) ~ CD11b_CD15 + brca1 + stage + refage, cluster = suid, data =  markers_ROIclust_o) %>%
  tbl_regression(exponentiate = TRUE,
                 include = CD11b_CD15) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
```

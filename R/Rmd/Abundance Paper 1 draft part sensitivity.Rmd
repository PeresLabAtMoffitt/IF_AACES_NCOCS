...
<span style="color: white;">Sensitivity analysis</span>
...
clust_markers$clusters_all_IandP <- factor(clust_markers$clusters_all_IandP,
                                           levels = c(1, 2, 3, 4, 5),
                                           labels = c("high", "mid", "mid-low", "mid-high", "low"))
...
clust_markers$dbl_pos <- factor(clust_markers$dbl_pos,
                                           levels = c(1, 2, 3, 4, 5),
                                           labels = c("high", "low", "mid-high", "mid-low", "mid"))
clust_markers <- clust_markers %>% mutate(dbl_pos = factor(dbl_pos, levels = c("low", "mid-low", "mid", "mid-high", "high")))
...
<span style="color: white;"> From here all is adjusted by debulking CA125</span>
...
```{r immunoscore per patients mean adjusted}
markers_ROI <- markers_ROI %>% 
  mutate(percentile_score_CD3_i = ntile(percent_CD3_total.i, 100) ) %>% 
  mutate(percentile_score_CD3_p = ntile(percent_CD3_total.p, 100) ) %>% 
  mutate(percentile_score_CD8_i = ntile(percent_CD8_total.i, 100) ) %>% 
...
```{r removing double stain from single stain count adjusted}
markers_ROI <- markers_ROI %>% 
  mutate(percent_CD3_total.i = percent_CD3_total.i - percent_CD3_CD8_total.i - percent_CD3_FoxP3_total.i) %>% 
  mutate(percent_CD8_total.i = percent_CD8_total.i - percent_CD3_CD8_total.i) %>% 
  mutate(percent_FoxP3_total.i = percent_FoxP3_total.i - percent_CD3_FoxP3_total.i) %>% 
...
# Table 3. Adjusted hazard ratios and 95% confidence intervals **Adjusted by dblkstat_CA125 + stage + refage + race**
## 1% group- absensece /presece
## Whole population
```{r}
markers_ROI_o <- markers_ROI %>% 
...
tbl1 <- coxph(Surv(time = markers_ROI$timelastfu_new, 
                   event = markers_ROI$surv_vital) ~ CD3 + dblkstat_CA125 + stage + refage + race, data =  markers_ROI_o)  %>% 
  tbl_regression(exponentiate = TRUE,
                 include = CD3) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl2 <- coxph(Surv(time = markers_ROI$timelastfu_new, 
                   event = markers_ROI$surv_vital) ~ CD3_CD8 + dblkstat_CA125 + stage + refage + race, data =  markers_ROI_o) %>%
  tbl_regression(exponentiate = TRUE,
                 include = CD3_CD8) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl3 <- coxph(Surv(time = markers_ROI$timelastfu_new, 
                   event = markers_ROI$surv_vital) ~ CD3_FoxP3 + dblkstat_CA125 + stage + refage + race, data =  markers_ROI_o) %>%
  tbl_regression(exponentiate = TRUE,
                 include = CD3_FoxP3) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl4 <- coxph(Surv(time = markers_ROI$timelastfu_new, 
                   event = markers_ROI$surv_vital) ~ CD11b + dblkstat_CA125 + stage + refage + race, data =  markers_ROI_o) %>%
  tbl_regression(exponentiate = TRUE,
                 include = CD11b) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl5 <- coxph(Surv(time = markers_ROI$timelastfu_new, 
                   event = markers_ROI$surv_vital) ~ CD11b_CD15 + dblkstat_CA125 + stage + refage + race, data =  markers_ROI_o) %>%
  tbl_regression(exponentiate = TRUE,
                 include = CD11b_CD15) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
...
tbl1 <- coxph(Surv(time = markers_ROI$timelastfu_new, 
                   event = markers_ROI$surv_vital) ~ CD3 + dblkstat_CA125 + stage + refage + race, data =  markers_ROI_t)  %>% 
  tbl_regression(exponentiate = TRUE,
                 include = CD3) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl2 <- coxph(Surv(time = markers_ROI$timelastfu_new, 
                   event = markers_ROI$surv_vital) ~ CD3_CD8 + dblkstat_CA125 + stage + refage + race, data =  markers_ROI_t) %>%
  tbl_regression(exponentiate = TRUE,
                 include = CD3_CD8) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl3 <- coxph(Surv(time = markers_ROI$timelastfu_new, 
                   event = markers_ROI$surv_vital) ~ CD3_FoxP3 + dblkstat_CA125 + stage + refage + race, data =  markers_ROI_t) %>%
  tbl_regression(exponentiate = TRUE,
                 include = CD3_FoxP3) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl4 <- coxph(Surv(time = markers_ROI$timelastfu_new, 
                   event = markers_ROI$surv_vital) ~ CD11b + dblkstat_CA125 + stage + refage + race, data =  markers_ROI_t) %>%
  tbl_regression(exponentiate = TRUE,
                 include = CD11b) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl5 <- coxph(Surv(time = markers_ROI$timelastfu_new, 
                   event = markers_ROI$surv_vital) ~ CD11b_CD15 + dblkstat_CA125 + stage + refage + race, data =  markers_ROI_t) %>%
  tbl_regression(exponentiate = TRUE,
                 include = CD11b_CD15) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
...
tbl1 <- coxph(Surv(time = markers_ROI$timelastfu_new, 
                   event = markers_ROI$surv_vital) ~ CD3 + dblkstat_CA125 + stage + refage + race, data =  markers_ROI_s) %>%
  tbl_regression(exponentiate = TRUE,
                 include = CD3) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl2 <- coxph(Surv(time = markers_ROI$timelastfu_new, 
                   event = markers_ROI$surv_vital) ~ CD3_CD8 + dblkstat_CA125 + stage + refage + race, data =  markers_ROI_s) %>%
  tbl_regression(exponentiate = TRUE,
                 include = CD3_CD8) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl3 <- coxph(Surv(time = markers_ROI$timelastfu_new, 
                   event = markers_ROI$surv_vital) ~ CD3_FoxP3 + dblkstat_CA125 + stage + refage + race, data =  markers_ROI_s) %>%
  tbl_regression(exponentiate = TRUE,
                 include = CD3_FoxP3) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl4 <- coxph(Surv(time = markers_ROI$timelastfu_new, 
                   event = markers_ROI$surv_vital) ~ CD11b + dblkstat_CA125 + stage + refage + race, data =  markers_ROI_s) %>%
  tbl_regression(exponentiate = TRUE,
                 include = CD11b) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl5 <- coxph(Surv(time = markers_ROI$timelastfu_new, 
                   event = markers_ROI$surv_vital) ~ CD11b_CD15 + dblkstat_CA125 + stage + refage + race, data =  markers_ROI_s) %>%
  tbl_regression(exponentiate = TRUE,
                 include = CD11b_CD15) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
...
```{r create group per sample ROI_global adjusted}
ROI_global <- ROI_global %>% 
...
```{r Table % cluster option adjusted}
markers_ROIclust_o <- ROI_global %>%
  filter(intratumoral_i_vs_peripheral_p_ == "Intratumoral") %>% 
  rename(CD3 = CD3_total, CD3_CD8 = CD3_CD8_total,
         CD3_FoxP3 = CD3_FoxP3_total, 
...
tbl1 <- coxph(Surv(time = markers_ROIclust_o$timelastfu_new, 
                   event = markers_ROIclust_o$surv_vital) ~ CD3 + dblkstat_CA125 + stage + refage + race, cluster = suid, data =  markers_ROIclust_o)  %>% 
  tbl_regression(exponentiate = TRUE,
                 include = CD3) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl2 <- coxph(Surv(time = markers_ROIclust_o$timelastfu_new, 
                   event = markers_ROIclust_o$surv_vital) ~ CD3_CD8 + dblkstat_CA125 + stage + refage + race, cluster = suid, data =  markers_ROIclust_o) %>%
  tbl_regression(exponentiate = TRUE,
                 include = CD3_CD8) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl3 <- coxph(Surv(time = markers_ROIclust_o$timelastfu_new, 
                   event = markers_ROIclust_o$surv_vital) ~ CD3_FoxP3 + dblkstat_CA125 + stage + refage + race, cluster = suid, data =  markers_ROIclust_o) %>%
  tbl_regression(exponentiate = TRUE,
                 include = CD3_FoxP3) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl4 <- coxph(Surv(time = markers_ROIclust_o$timelastfu_new, 
                   event = markers_ROIclust_o$surv_vital) ~ CD11b + dblkstat_CA125 + stage + refage + race, cluster = suid, data =  markers_ROIclust_o) %>%
  tbl_regression(exponentiate = TRUE,
                 include = CD11b) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl5 <- coxph(Surv(time = markers_ROIclust_o$timelastfu_new, 
                   event = markers_ROIclust_o$surv_vital) ~ CD11b_CD15 + dblkstat_CA125 + stage + refage + race, cluster = suid, data =  markers_ROIclust_o) %>%
  tbl_regression(exponentiate = TRUE,
                 include = CD11b_CD15) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
...
tbl1 <- coxph(Surv(time = markers_ROIclust_t$timelastfu_new, 
                   event = markers_ROIclust_t$surv_vital) ~ CD3 + dblkstat_CA125 + stage + refage + race, cluster = suid, data =  markers_ROIclust_t)  %>% 
  tbl_regression(exponentiate = TRUE,
                 include = CD3) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl2 <- coxph(Surv(time = markers_ROIclust_t$timelastfu_new, 
                   event = markers_ROIclust_t$surv_vital) ~ CD3_CD8 + dblkstat_CA125 + stage + refage + race, cluster = suid, data =  markers_ROIclust_t) %>%
  tbl_regression(exponentiate = TRUE,
                 include = CD3_CD8) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl3 <- coxph(Surv(time = markers_ROIclust_t$timelastfu_new, 
                   event = markers_ROIclust_t$surv_vital) ~ CD3_FoxP3 + dblkstat_CA125 + stage + refage + race, cluster = suid, data =  markers_ROIclust_t) %>%
  tbl_regression(exponentiate = TRUE,
                 include = CD3_FoxP3) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl4 <- coxph(Surv(time = markers_ROIclust_t$timelastfu_new, 
                   event = markers_ROIclust_t$surv_vital) ~ CD11b + dblkstat_CA125 + stage + refage + race, cluster = suid, data =  markers_ROIclust_t) %>%
  tbl_regression(exponentiate = TRUE,
                 include = CD11b) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl5 <- coxph(Surv(time = markers_ROIclust_t$timelastfu_new, 
                   event = markers_ROIclust_t$surv_vital) ~ CD11b_CD15 + dblkstat_CA125 + stage + refage + race, cluster = suid, data =  markers_ROIclust_t) %>%
  tbl_regression(exponentiate = TRUE,
                 include = CD11b_CD15) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
...
tbl1 <- coxph(Surv(time = markers_ROIclust_s$timelastfu_new, 
                   event = markers_ROIclust_s$surv_vital) ~ CD3 + dblkstat_CA125 + stage + refage + race, cluster = suid, data =  markers_ROIclust_s) %>%
  tbl_regression(exponentiate = TRUE,
                 include = CD3) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl2 <- coxph(Surv(time = markers_ROIclust_s$timelastfu_new, 
                   event = markers_ROIclust_s$surv_vital) ~ CD3_CD8 + dblkstat_CA125 + stage + refage + race, cluster = suid, data =  markers_ROIclust_s) %>%
  tbl_regression(exponentiate = TRUE,
                 include = CD3_CD8) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl3 <- coxph(Surv(time = markers_ROIclust_s$timelastfu_new, 
                   event = markers_ROIclust_s$surv_vital) ~ CD3_FoxP3 + dblkstat_CA125 + stage + refage + race, cluster = suid, data =  markers_ROIclust_s) %>%
  tbl_regression(exponentiate = TRUE,
                 include = CD3_FoxP3) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl4 <- coxph(Surv(time = markers_ROIclust_s$timelastfu_new, 
                   event = markers_ROIclust_s$surv_vital) ~ CD11b + dblkstat_CA125 + stage + refage + race, cluster = suid, data =  markers_ROIclust_s) %>%
  tbl_regression(exponentiate = TRUE,
                 include = CD11b) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl5 <- coxph(Surv(time = markers_ROIclust_s$timelastfu_new, 
                   event = markers_ROIclust_s$surv_vital) ~ CD11b_CD15 + dblkstat_CA125 + stage + refage + race, cluster = suid, data =  markers_ROIclust_s) %>%
  tbl_regression(exponentiate = TRUE,
                 include = CD11b_CD15) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
...
## In White (I can bind W/B later if you want them side by side)
```{r Table % cluster option white adjusted}
markers_ROIclust_o <- ROI_global %>%
  filter(intratumoral_i_vs_peripheral_p_ == "Intratumoral" & race == "White") %>% 
  rename(CD3 = CD3_total, CD3_CD8 = CD3_CD8_total,
         CD3_FoxP3 = CD3_FoxP3_total, 
...
tbl1 <- coxph(Surv(time = markers_ROIclust_o$timelastfu_new, 
                   event = markers_ROIclust_o$surv_vital) ~ CD3 + dblkstat_CA125 + stage + refage, cluster = suid, data =  markers_ROIclust_o)  %>% 
  tbl_regression(exponentiate = TRUE,
                 include = CD3) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl2 <- coxph(Surv(time = markers_ROIclust_o$timelastfu_new, 
                   event = markers_ROIclust_o$surv_vital) ~ CD3_CD8 + dblkstat_CA125 + stage + refage, cluster = suid, data =  markers_ROIclust_o) %>%
  tbl_regression(exponentiate = TRUE,
                 include = CD3_CD8) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl3 <- coxph(Surv(time = markers_ROIclust_o$timelastfu_new, 
                   event = markers_ROIclust_o$surv_vital) ~ CD3_FoxP3 + dblkstat_CA125 + stage + refage, cluster = suid, data =  markers_ROIclust_o) %>%
  tbl_regression(exponentiate = TRUE,
                 include = CD3_FoxP3) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl4 <- coxph(Surv(time = markers_ROIclust_o$timelastfu_new, 
                   event = markers_ROIclust_o$surv_vital) ~ CD11b + dblkstat_CA125 + stage + refage, cluster = suid, data =  markers_ROIclust_o) %>%
  tbl_regression(exponentiate = TRUE,
                 include = CD11b) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl5 <- coxph(Surv(time = markers_ROIclust_o$timelastfu_new, 
                   event = markers_ROIclust_o$surv_vital) ~ CD11b_CD15 + dblkstat_CA125 + stage + refage, cluster = suid, data =  markers_ROIclust_o) %>%
  tbl_regression(exponentiate = TRUE,
                 include = CD11b_CD15) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
...
tbl1 <- coxph(Surv(time = markers_ROIclust_t$timelastfu_new, 
                   event = markers_ROIclust_t$surv_vital) ~ CD3 + dblkstat_CA125 + stage + refage, cluster = suid, data =  markers_ROIclust_t)  %>% 
  tbl_regression(exponentiate = TRUE,
                 include = CD3) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl2 <- coxph(Surv(time = markers_ROIclust_t$timelastfu_new, 
                   event = markers_ROIclust_t$surv_vital) ~ CD3_CD8 + dblkstat_CA125 + stage + refage, cluster = suid, data =  markers_ROIclust_t) %>%
  tbl_regression(exponentiate = TRUE,
                 include = CD3_CD8) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl3 <- coxph(Surv(time = markers_ROIclust_t$timelastfu_new, 
                   event = markers_ROIclust_t$surv_vital) ~ CD3_FoxP3 + dblkstat_CA125 + stage + refage, cluster = suid, data =  markers_ROIclust_t) %>%
  tbl_regression(exponentiate = TRUE,
                 include = CD3_FoxP3) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl4 <- coxph(Surv(time = markers_ROIclust_t$timelastfu_new, 
                   event = markers_ROIclust_t$surv_vital) ~ CD11b + dblkstat_CA125 + stage + refage, cluster = suid, data =  markers_ROIclust_t) %>%
  tbl_regression(exponentiate = TRUE,
                 include = CD11b) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl5 <- coxph(Surv(time = markers_ROIclust_t$timelastfu_new, 
                   event = markers_ROIclust_t$surv_vital) ~ CD11b_CD15 + dblkstat_CA125 + stage + refage, cluster = suid, data =  markers_ROIclust_t) %>%
  tbl_regression(exponentiate = TRUE,
                 include = CD11b_CD15) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
...
tbl1 <- coxph(Surv(time = markers_ROIclust_s$timelastfu_new, 
                   event = markers_ROIclust_s$surv_vital) ~ CD3 + dblkstat_CA125 + stage + refage, cluster = suid, data =  markers_ROIclust_s) %>%
  tbl_regression(exponentiate = TRUE,
                 include = CD3) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl2 <- coxph(Surv(time = markers_ROIclust_s$timelastfu_new, 
                   event = markers_ROIclust_s$surv_vital) ~ CD3_CD8 + dblkstat_CA125 + stage + refage, cluster = suid, data =  markers_ROIclust_s) %>%
  tbl_regression(exponentiate = TRUE,
                 include = CD3_CD8) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl3 <- coxph(Surv(time = markers_ROIclust_s$timelastfu_new, 
                   event = markers_ROIclust_s$surv_vital) ~ CD3_FoxP3 + dblkstat_CA125 + stage + refage, cluster = suid, data =  markers_ROIclust_s) %>%
  tbl_regression(exponentiate = TRUE,
                 include = CD3_FoxP3) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl4 <- coxph(Surv(time = markers_ROIclust_s$timelastfu_new, 
                   event = markers_ROIclust_s$surv_vital) ~ CD11b + dblkstat_CA125 + stage + refage, cluster = suid, data =  markers_ROIclust_s) %>%
  tbl_regression(exponentiate = TRUE,
                 include = CD11b) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl5 <- coxph(Surv(time = markers_ROIclust_s$timelastfu_new, 
                   event = markers_ROIclust_s$surv_vital) ~ CD11b_CD15 + dblkstat_CA125 + stage + refage, cluster = suid, data =  markers_ROIclust_s) %>%
  tbl_regression(exponentiate = TRUE,
                 include = CD11b_CD15) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
...
## In Black
```{r Table % cluster option black adjusted}
markers_ROIclust_o <- ROI_global %>%
  filter(intratumoral_i_vs_peripheral_p_ == "Intratumoral" & race == "Black") %>% 
  rename(CD3 = CD3_total, CD3_CD8 = CD3_CD8_total,
         CD3_FoxP3 = CD3_FoxP3_total, 
...
# HR interaction terms by race
Cross product interaction terms between race and the immune markers group.  
Adjusted for dblkstat_CA125, stage, age and race and with cluster option.  
`r emo::ji("question_mark")` Do you want high with White or with Black? or each comparison? I think it is the later but what is the thought behind that?


<!-- inline_text(tbl1, variable = "CD3:race", level = "high * White", pattern = "{p.value}", pvalue_fun = purrr::partial(style_pvalue, digits = 2)) -->

```{r Table interactions by race cluster option adjusted}
markers_ROIclust_o <- ROI_global %>%
  filter(intratumoral_i_vs_peripheral_p_ == "Intratumoral") %>% 
  rename(CD3 = CD3_total, CD3_CD8 = CD3_CD8_total,
         CD3_FoxP3 = CD3_FoxP3_total, 
...
tbl1 <- coxph(Surv(time = markers_ROIclust_o$timelastfu_new,
                   event = markers_ROIclust_o$surv_vital) ~ CD3 + dblkstat_CA125 + stage + refage + race + CD3*race, cluster = suid, data =  markers_ROIclust_o)  %>% 
  tbl_regression(exponentiate = TRUE,
                 include = !CD3:race, pvalue_fun = purrr::partial(style_pvalue, digits = 2)) %>% 
  bold_p(t = .05) %>% 
  modify_column_hide(c(estimate, ci))
...
tbl2 <- coxph(Surv(time = markers_ROIclust_o$timelastfu_new, 
                   event = markers_ROIclust_o$surv_vital) ~ CD3_CD8 + dblkstat_CA125 + stage + refage + race + CD3_CD8*race, cluster = suid, data =  markers_ROIclust_o) %>%
  tbl_regression(exponentiate = TRUE,
                 include = !CD3_CD8:race, pvalue_fun = purrr::partial(style_pvalue, digits = 2)) %>% 
  bold_p(t = .05) %>% 
  modify_column_hide(c(estimate, ci))
...
tbl3 <- coxph(Surv(time = markers_ROIclust_o$timelastfu_new, 
                   event = markers_ROIclust_o$surv_vital) ~ CD3_FoxP3 + dblkstat_CA125 + stage + refage + race + CD3_FoxP3*race, cluster = suid, data =  markers_ROIclust_o) %>%
  tbl_regression(exponentiate = TRUE,
                 include = !CD3_FoxP3:race, pvalue_fun = purrr::partial(style_pvalue, digits = 2)) %>% 
  bold_p(t = .05) %>% 
  modify_column_hide(c(estimate, ci))
...
tbl4 <- coxph(Surv(time = markers_ROIclust_o$timelastfu_new, 
                   event = markers_ROIclust_o$surv_vital) ~ CD11b + dblkstat_CA125 + stage + refage + race + CD11b*race, cluster = suid, data =  markers_ROIclust_o) %>%
  tbl_regression(exponentiate = TRUE,
                 include = !CD11b:race, pvalue_fun = purrr::partial(style_pvalue, digits = 2)) %>% 
  bold_p(t = .05) %>% 
  modify_column_hide(c(estimate, ci))
...
tbl5 <- coxph(Surv(time = markers_ROIclust_o$timelastfu_new, 
                   event = markers_ROIclust_o$surv_vital) ~ CD11b_CD15 + dblkstat_CA125 + stage + refage + race + CD11b_CD15*race, cluster = suid, data =  markers_ROIclust_o) %>%
  tbl_regression(exponentiate = TRUE,
                 include = !CD11b_CD15:race, pvalue_fun = purrr::partial(style_pvalue, digits = 2)) %>% 
  bold_p(t = .05) %>% 
  modify_column_hide(c(estimate, ci))
...
tbl1 <- coxph(Surv(time = markers_ROIclust_t$timelastfu_new, 
                   event = markers_ROIclust_t$surv_vital) ~ CD3 + dblkstat_CA125 + stage + refage + race + CD3*race, cluster = suid, data =  markers_ROIclust_t)  %>% 
  tbl_regression(exponentiate = TRUE,
                 include = !CD3:race, pvalue_fun = purrr::partial(style_pvalue, digits = 2)) %>%
  bold_p(t = .05) %>% 
  modify_column_hide(c(estimate, ci))
tbl2 <- coxph(Surv(time = markers_ROIclust_t$timelastfu_new, 
                   event = markers_ROIclust_t$surv_vital) ~ CD3_CD8 + dblkstat_CA125 + stage + refage + race + CD3_CD8*race, cluster = suid, data =  markers_ROIclust_t) %>%
  tbl_regression(exponentiate = TRUE,
                 include = !CD3_CD8:race, pvalue_fun = purrr::partial(style_pvalue, digits = 2)) %>% 
  bold_p(t = .05) %>% 
  modify_column_hide(c(estimate, ci))
tbl3 <- coxph(Surv(time = markers_ROIclust_t$timelastfu_new, 
                   event = markers_ROIclust_t$surv_vital) ~ CD3_FoxP3 + dblkstat_CA125 + stage + refage + race + CD3_FoxP3*race, cluster = suid, data =  markers_ROIclust_t) %>%
  tbl_regression(exponentiate = TRUE,
                 include = !CD3_FoxP3:race, pvalue_fun = purrr::partial(style_pvalue, digits = 2)) %>% 
  bold_p(t = .05) %>% 
  modify_column_hide(c(estimate, ci))
tbl4 <- coxph(Surv(time = markers_ROIclust_t$timelastfu_new, 
                   event = markers_ROIclust_t$surv_vital) ~ CD11b + dblkstat_CA125 + stage + refage + race + CD11b*race, cluster = suid, data =  markers_ROIclust_t) %>%
  tbl_regression(exponentiate = TRUE,
                 include = !CD11b:race, pvalue_fun = purrr::partial(style_pvalue, digits = 2)) %>% 
  bold_p(t = .05) %>% 
  modify_column_hide(c(estimate, ci))
tbl5 <- coxph(Surv(time = markers_ROIclust_t$timelastfu_new, 
                   event = markers_ROIclust_t$surv_vital) ~ CD11b_CD15 + dblkstat_CA125 + stage + refage + race + CD11b_CD15*race, cluster = suid, data =  markers_ROIclust_t) %>%
  tbl_regression(exponentiate = TRUE,
                 include = !CD11b_CD15:race, pvalue_fun = purrr::partial(style_pvalue, digits = 2)) %>% 
  bold_p(t = .05) %>% 
  modify_column_hide(c(estimate, ci))
...
tbl1 <- coxph(Surv(time = markers_ROIclust_s$timelastfu_new, 
                   event = markers_ROIclust_s$surv_vital) ~ CD3 + dblkstat_CA125 + stage + refage + race + CD3*race, cluster = suid, data =  markers_ROIclust_s) %>%
  tbl_regression(exponentiate = TRUE,
                 include = !CD3:race, pvalue_fun = purrr::partial(style_pvalue, digits = 2)) %>% 
  bold_p(t = .05) %>% 
  modify_column_hide(c(estimate, ci))
tbl2 <- coxph(Surv(time = markers_ROIclust_s$timelastfu_new, 
                   event = markers_ROIclust_s$surv_vital) ~ CD3_CD8 + dblkstat_CA125 + stage + refage + race + CD3_CD8*race, cluster = suid, data =  markers_ROIclust_s) %>%
  tbl_regression(exponentiate = TRUE,
                 include = !CD3_CD8:race, pvalue_fun = purrr::partial(style_pvalue, digits = 2)) %>% 
  bold_p(t = .05) %>% 
  modify_column_hide(c(estimate, ci))
tbl3 <- coxph(Surv(time = markers_ROIclust_s$timelastfu_new, 
                   event = markers_ROIclust_s$surv_vital) ~ CD3_FoxP3 + dblkstat_CA125 + stage + refage + race + CD3_FoxP3*race, cluster = suid, data =  markers_ROIclust_s) %>%
  tbl_regression(exponentiate = TRUE,
                 include = !CD3_FoxP3:race, pvalue_fun = purrr::partial(style_pvalue, digits = 2)) %>% 
  bold_p(t = .05) %>% 
  modify_column_hide(c(estimate, ci))
tbl4 <- coxph(Surv(time = markers_ROIclust_s$timelastfu_new, 
                   event = markers_ROIclust_s$surv_vital) ~ CD11b + dblkstat_CA125 + stage + refage + race + CD11b*race, cluster = suid, data =  markers_ROIclust_s) %>%
  tbl_regression(exponentiate = TRUE,
                 include = !CD11b:race, pvalue_fun = purrr::partial(style_pvalue, digits = 2)) %>% 
  bold_p(t = .05) %>% 
  modify_column_hide(c(estimate, ci))
tbl5 <- coxph(Surv(time = markers_ROIclust_s$timelastfu_new, 
                   event = markers_ROIclust_s$surv_vital) ~ CD11b_CD15 + dblkstat_CA125 + stage + refage + race + CD11b_CD15*race, cluster = suid, data =  markers_ROIclust_s) %>%
  tbl_regression(exponentiate = TRUE,
                 include = !CD11b_CD15:race, pvalue_fun = purrr::partial(style_pvalue, digits = 2)) %>% 
  bold_p(t = .05) %>% 
  modify_column_hide(c(estimate, ci))
...
`r emo::ji("pad")` Note to remember. The cluster were built on the data with the averaged measurement. I am not using the cluster option in the coxph even though I saw a really slight change in CI values but not in p value.  
## Overall
```{r Table HR cluster adjusted}
  
tbl1 <- coxph(Surv(time = markers_ROI$timelastfu_new, 
                   event = markers_ROI$surv_vital) ~ clusters_all_IandP + dblkstat_CA125 + stage + refage + race, data =  markers_ROI)  %>% 
  tbl_regression(exponentiate = TRUE,
                 include = clusters_all_IandP) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl2 <- coxph(Surv(time = markers_ROI$timelastfu_new, 
                   event = markers_ROI$surv_vital) ~ dbl_pos + dblkstat_CA125 + stage + refage + race, data =  markers_ROI) %>%
  tbl_regression(exponentiate = TRUE,
                 include = dbl_pos) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl3 <- coxph(Surv(time = markers_ROI$timelastfu_new, 
                   event = markers_ROI$surv_vital) ~ clusters_CD3CD8 + dblkstat_CA125 + stage + refage + race, data =  markers_ROI) %>%
  tbl_regression(exponentiate = TRUE,
                 include = clusters_CD3CD8) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl4 <- coxph(Surv(time = markers_ROI$timelastfu_new, 
                   event = markers_ROI$surv_vital) ~ clusters_CD3FoxP3 + dblkstat_CA125 + stage + refage + race, data =  markers_ROI) %>%
  tbl_regression(exponentiate = TRUE,
                 include = clusters_CD3FoxP3) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl5 <- coxph(Surv(time = markers_ROI$timelastfu_new, 
                   event = markers_ROI$surv_vital) ~ clusters_FoxP3_ + dblkstat_CA125 + stage + refage + race, data =  markers_ROI) %>%
  tbl_regression(exponentiate = TRUE,
                 include = clusters_FoxP3_) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl6 <- coxph(Surv(time = markers_ROI$timelastfu_new, 
                   event = markers_ROI$surv_vital) ~ clusters_CD11bCD15 + dblkstat_CA125 + stage + refage + race, data =  markers_ROI) %>%
  tbl_regression(exponentiate = TRUE,
                 include = clusters_CD11bCD15) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
...
## In White (I can bind W/B if you want them side by side)
```{r Table HR white adjusted}
markers_ROI_white <- markers_ROI %>%
  filter(race == "White")
  
tbl1 <- coxph(Surv(time = markers_ROI_white$timelastfu_new, 
                   event = markers_ROI_white$surv_vital) ~ clusters_all_IandP + dblkstat_CA125 + stage + refage, data =  markers_ROI_white)  %>% 
  tbl_regression(exponentiate = TRUE,
                 include = clusters_all_IandP) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl2 <- coxph(Surv(time = markers_ROI_white$timelastfu_new, 
                   event = markers_ROI_white$surv_vital) ~ dbl_pos + dblkstat_CA125 + stage + refage, data =  markers_ROI_white) %>%
  tbl_regression(exponentiate = TRUE,
                 include = dbl_pos) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl3 <- coxph(Surv(time = markers_ROI_white$timelastfu_new, 
                   event = markers_ROI_white$surv_vital) ~ clusters_CD3CD8 + dblkstat_CA125 + stage + refage, data =  markers_ROI_white) %>%
  tbl_regression(exponentiate = TRUE,
                 include = clusters_CD3CD8) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl4 <- coxph(Surv(time = markers_ROI_white$timelastfu_new, 
                   event = markers_ROI_white$surv_vital) ~ clusters_CD3FoxP3 + dblkstat_CA125 + stage + refage, data =  markers_ROI_white) %>%
  tbl_regression(exponentiate = TRUE,
                 include = clusters_CD3FoxP3) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl5 <- coxph(Surv(time = markers_ROI_white$timelastfu_new, 
                   event = markers_ROI_white$surv_vital) ~ clusters_FoxP3_ + dblkstat_CA125 + stage + refage, data =  markers_ROI_white) %>%
  tbl_regression(exponentiate = TRUE,
                 include = clusters_FoxP3_) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl6 <- coxph(Surv(time = markers_ROI_white$timelastfu_new, 
                   event = markers_ROI_white$surv_vital) ~ clusters_CD11bCD15 + dblkstat_CA125 + stage + refage, data =  markers_ROI_white) %>%
  tbl_regression(exponentiate = TRUE,
                 include = clusters_CD11bCD15) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
...
## In Black
```{r Table cluster black adjusted}
markers_ROI_black <- markers_ROI %>%
  filter(race == "Black")
  
tbl1 <- coxph(Surv(time = markers_ROI_black$timelastfu_new, 
                   event = markers_ROI_black$surv_vital) ~ clusters_all_IandP + dblkstat_CA125 + stage + refage, data =  markers_ROI_black)  %>% 
  tbl_regression(exponentiate = TRUE,
                 include = clusters_all_IandP) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl2 <- coxph(Surv(time = markers_ROI_black$timelastfu_new, 
                   event = markers_ROI_black$surv_vital) ~ dbl_pos + dblkstat_CA125 + stage + refage, data =  markers_ROI_black) %>%
  tbl_regression(exponentiate = TRUE,
                 include = dbl_pos) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl3 <- coxph(Surv(time = markers_ROI_black$timelastfu_new, 
                   event = markers_ROI_black$surv_vital) ~ clusters_CD3CD8 + dblkstat_CA125 + stage + refage, data =  markers_ROI_black) %>%
  tbl_regression(exponentiate = TRUE,
                 include = clusters_CD3CD8) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl4 <- coxph(Surv(time = markers_ROI_black$timelastfu_new, 
                   event = markers_ROI_black$surv_vital) ~ clusters_CD3FoxP3 + dblkstat_CA125 + stage + refage, data =  markers_ROI_black) %>%
  tbl_regression(exponentiate = TRUE,
                 include = clusters_CD3FoxP3) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl5 <- coxph(Surv(time = markers_ROI_black$timelastfu_new, 
                   event = markers_ROI_black$surv_vital) ~ clusters_FoxP3_ + dblkstat_CA125 + stage + refage, data =  markers_ROI_black) %>%
  tbl_regression(exponentiate = TRUE,
                 include = clusters_FoxP3_) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl6 <- coxph(Surv(time = markers_ROI_black$timelastfu_new, 
                   event = markers_ROI_black$surv_vital) ~ clusters_CD11bCD15 + dblkstat_CA125 + stage + refage, data =  markers_ROI_black) %>%
  tbl_regression(exponentiate = TRUE,
                 include = clusters_CD11bCD15) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
...
`r emo::ji("pad")` Note to remember. The immunoscore were built on the data with the averaged measurement. I am not using the cluster option in the coxph even though I saw a really slight change in CI and p value.  
## Overall
```{r Table HR Immunoscore adjusted}
  
tbl1 <- coxph(Surv(time = markers_ROI$timelastfu_new, 
                   event = markers_ROI$surv_vital) ~ immunoscore_patients + dblkstat_CA125 + stage + refage + race, data =  markers_ROI)  %>% 
  tbl_regression(exponentiate = TRUE,
                 include = immunoscore_patients) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl2 <- coxph(Surv(time = markers_ROI$timelastfu_new, 
                   event = markers_ROI$surv_vital) ~ immunoscore_2018lancet_patients + dblkstat_CA125 + stage + refage + race, data =  markers_ROI) %>%
  tbl_regression(exponentiate = TRUE,
                 include = immunoscore_2018lancet_patients) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
...
## In White (I can bind W/B if you want them side by side)
```{r Table HR Immunoscore white adjusted}
markers_ROI_white <- markers_ROI %>%
  filter(race == "White")
  
tbl1 <- coxph(Surv(time = markers_ROI_white$timelastfu_new, 
                   event = markers_ROI_white$surv_vital) ~ immunoscore_patients + dblkstat_CA125 + stage + refage, data =  markers_ROI_white)  %>% 
  tbl_regression(exponentiate = TRUE,
                 include = immunoscore_patients) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl2 <- coxph(Surv(time = markers_ROI_white$timelastfu_new, 
                   event = markers_ROI_white$surv_vital) ~ immunoscore_2018lancet_patients + dblkstat_CA125 + stage + refage, data =  markers_ROI_white) %>%
  tbl_regression(exponentiate = TRUE,
                 include = immunoscore_2018lancet_patients) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
...
## In Black
```{r Table HR Immunoscore black adjusted}
markers_ROI_black <- markers_ROI %>%
  filter(race == "Black")
  
tbl1 <- coxph(Surv(time = markers_ROI_black$timelastfu_new, 
                   event = markers_ROI_black$surv_vital) ~ immunoscore_patients + dblkstat_CA125 + stage + refage, data =  markers_ROI_black)  %>% 
  tbl_regression(exponentiate = TRUE,
                 include = immunoscore_patients) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
tbl2 <- coxph(Surv(time = markers_ROI_black$timelastfu_new, 
                   event = markers_ROI_black$surv_vital) ~ immunoscore_2018lancet_patients + dblkstat_CA125 + stage + refage, data =  markers_ROI_black) %>%
  tbl_regression(exponentiate = TRUE,
                 include = immunoscore_2018lancet_patients) %>% bold_p(t = .05) %>% add_nevent(location = "level") %>% add_n(location = "level")
